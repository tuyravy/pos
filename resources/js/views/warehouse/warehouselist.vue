<template>
  <div class="app-container">
    ...
    <el-card>
      <el-tabs v-model="activeActivity">
        <el-tab-pane :label="$t('WarehouseList')" name="first" class="active">
          <el-table
            v-loading="loading"
            :data="list"
            border
          ...
          >
            ...
            <el-table-column align="left" :label="$t('table.BrName')" width="150">
              <template slot-scope="scope">
                <span>{{ scope.row.BrName }}</span>
              </template>
            </el-table-column>
            <el-table-column align="left" :label="$t('table.WarehouseName')" width="180">
              <template slot-scope="scope">
                <span>{{ scope.row.warehouse_name }}</span>
              </template>
            </el-table-column>
            <el-table-column align="left" :label="$t('table.WarehouseNote')" width="200">
              <template slot-scope="scope">
                <span>{{ scope.row.warehouse_note }}</span>
              </template>
            </el-table-column>
            ...
            <el-table-column align="center" :label="$t('table.Actions')" width="180">
              <template slot-scope="scope">
                <router-link :to="'/warehouse/updatewarehouse/edit/' + scope.row.warehouse_id">
                ...
                <el-button
                  ...
                  type="success"
                  icon="el-icon-check"
                  @click="handleApprove(scope.row.warehouse_id)"
                />
                <el-button
...
const AuthorizeResource = new Resource('warehouse/authorized');
...
export default {
  ...
  methods: {
    async getList() {
      ...
      this.loading = true;
      const { data, meta } = await WarehouseResource.list(this.query);
      this.list = data;
    ...
    handleDelete(row) {
      this.$confirm(`Are you sure you want to delete?`, '', { type: 'warning' })
        .then(() => {
          WarehouseResource.destroy(row);
          this.$message({
            message: 'Warehouse has been delete successfully',
            type: 'success',
            duration: 5 * 1000,
          });
          this.getList();
    ...
    handleApprove(Id) {
      this.loginForm.warehouse_id = Id;
